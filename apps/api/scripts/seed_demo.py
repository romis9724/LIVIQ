...
공지 데모 86건(scripts/data/notices_demo.py — 공지사항샘플정리.xlsx 6건 + 공지사항추가 시트
80건) — published 게시글로 멱등 upsert(제목 기준). category는 NOTICE_CATEGORY 시드 코드
(liviq_db.codes_seed)와 정합하고, target_dong이 있으면 그 동만 대상으로 표시한다.
...
async def _building_ids(session: AsyncSession) -> dict[str, uuid.UUID]:
    """동 name → building id 매핑(공지 target_buildings 해석용 — 공지마다 조회하지 않는다)."""
    rows = await session.execute(
        select(Building.name, Building.id).where(Building.tenant_id == DEV_TENANT_ID)
    )
    return {name: building_id for name, building_id in rows}


def _target_buildings(
    seed_dong: str | None, building_ids: dict[str, uuid.UUID]
) -> list[str] | None:
    """target_dong → target_buildings(동 1개) 또는 None(전체동).

    없는 동은 조용히 넘기지 않고 중단한다 — 잘못된 데모 데이터가 전체동 공지로 새지 않게.
    """
    if seed_dong is None:
        return None
    building_id = building_ids.get(seed_dong)
    if building_id is None:
        raise SystemExit(
            f"공지 대상 동을 찾을 수 없습니다: {seed_dong} — 먼저 명부 시드를 실행하세요."
        )
...
async def _seed_notices(
    ...
    building_ids: dict[str, uuid.UUID],
    published_by: uuid.UUID | None,
) -> None:
    """공지 데모 멱등 upsert(제목 기준) — published 게시글(target_dong 있으면 해당 동 대상)."""
    for seed in NOTICES:
        exists = await session.scalar(
        ...
        session.add(
            Notice(
                ...
                target_buildings=_target_buildings(seed.target_dong, building_ids),
...
async def main() -> None:
    ...
    try:
        async with factory() as session, session.begin():
            ...
            building_ids = await _building_ids(session)
            published_by = await _manager_user_id(session, crypto)
            await _seed_notices(session, category_ids, building_ids, published_by)
