"""find_nearest_available_parking — 본인 동에서 가까운 빈 주차자리 top_k (H15-4, ADR-0023).

세대→동(PG authz)로 앵커 코어를 정하고, `parking_layouts`(면·코어)와 `parking_occupancy`
(점유 스냅샷, SoR)를 읽어 `geometry.nearest_available_spots`로 최근접 빈 면을 계산한다.
...
점유는 데모 데이터이므로 답변에 그 사실을 명시한다(규칙 1 — 출처=parking_occupancy).
"""
...
_OCCUPANCY_SQL = text("SELECT spot_no FROM parking_occupancy WHERE tenant_id = :tid")
...
def _quote(nearest: list[Any]) -> str:
    ...
    body = "\n".join(lines)
    return f"{body}\n(데모 데이터 · 출처: parking_occupancy 점유 스냅샷)"
