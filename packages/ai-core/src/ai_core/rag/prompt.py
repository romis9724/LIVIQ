...
# 인용 누락 재요청(H15-2 R21) — 근거를 **만들지 않고 귀속만** 요구한다.
# 실측: 정답을 정확히 쓰고도 [n]을 붙이지 않아 폐기된 답변이 Critical 180에서 31~37건이었다.
# 근거 없는 문장은 추가가 아니라 **삭제**를 지시한다 — 지어내기를 유도하면 규칙 1을 깬다.
CITATION_RETRY_PROMPT = """직전 답변에 근거 번호가 없어 사용자에게 전달할 수 없습니다.
같은 내용을 유지한 채로, 각 문장이 사용한 근거의 [번호]를 문장 끝에 붙여 다시 출력하십시오.
- 근거에 없는 내용을 새로 추가하지 마십시오.
- 어느 근거에도 해당하지 않는 문장은 삭제하십시오.
- 남길 내용이 없으면 정확히 "NO_EVIDENCE"라고만 답하십시오.
- 답변 본문만 출력하고 설명·머리말은 붙이지 마십시오."""
