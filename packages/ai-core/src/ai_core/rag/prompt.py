...
# 규칙 4는 분량 지시만 강화한 것(사용자 요청 2026-08-01, "짧고 간결하게") — 인용·NO_EVIDENCE
# 지시(규칙 1~3)는 손대지 않는다. 8B의 [n] 부착률이 최대 병목이라 희석시키면 안 된다(H15-2).
...
3. 근거로 답할 수 없으면 정확히 "NO_EVIDENCE"라고만 답하십시오.
4. 한국어로 핵심만 2~4문장으로 답하고, 항목이 여럿이면 짧은 목록으로 쓰십시오.
