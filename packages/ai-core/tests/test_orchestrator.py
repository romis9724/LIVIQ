...
async def test_missing_citations_triggers_one_retry(settings: AiCoreSettings) -> None:
    """근거를 찾았는데 모델이 [n]을 안 붙인 답변은 폐기 대신 **1회 재요청**한다.

    실측(H15-2 R20): Critical 180에서 정답을 정확히 쓰고도 마커 누락으로 폐기된 건이
    31~37건(17~21%)이었다. 근거는 유사도 1위로 찾아왔고 폴백 전건이 도구를 호출했다 —
    검색 실패가 아니라 인용 규율 실패다. 프롬프트 강화로는 개선되지 않았다(기각).
    """
    retry_prompts: list[str] = []

    def decide(messages: list[dict[str, Any]]) -> dict[str, Any]:
        # 재요청 turn은 대화 마지막이 user(인용 지시)이고 tools 없이 온다.
        last = messages[-1]
        if last.get("role") == "user" and "번호" in str(last.get("content", "")):
            retry_prompts.append(str(last["content"]))
            return _decision(content="지하주차장은 24시간 개방합니다. [1]")
        if any(m.get("role") == "tool" for m in messages):
            return _decision(content="")
        return _decision(tool_calls=[_tc("search_documents", {"query": "주차"})])

    retriever = FakeRetriever([_chunk()])
    # 최종 답변 turn은 인용 없이 답한다 → 재요청 경로로 들어가야 한다.
    llm = _agent_llm(settings, decide, answer="지하주차장은 24시간 개방합니다.")
    events = await _run(llm, retriever)
    done = _done(events)

    assert retry_prompts, "인용 누락인데 재요청을 하지 않았다"
    assert done.status == "answered"
    assert done.citations and done.citations[0].ref == 1
    assert "[1]" in done.answer


async def test_retry_failure_still_falls_back(settings: AiCoreSettings) -> None:
    """재요청도 인용을 붙이지 못하면 기존대로 폴백한다 — 규칙 1을 우회하지 않는다."""

    def decide(messages: list[dict[str, Any]]) -> dict[str, Any]:
        last = messages[-1]
        if last.get("role") == "user" and "번호" in str(last.get("content", "")):
            return _decision(content="여전히 인용 없는 답변입니다.")
        if any(m.get("role") == "tool" for m in messages):
            return _decision(content="")
        return _decision(tool_calls=[_tc("search_documents", {"query": "주차"})])
...
async def test_retry_is_skipped_when_tool_cards_carry_the_evidence(
    settings: AiCoreSettings,
) -> None:
    """도구 카드(확정 데이터)가 근거면 [n]이 없어도 통과하던 경로는 그대로 — 재요청 없음."""
    retry_seen: list[str] = []

    def decide(messages: list[dict[str, Any]]) -> dict[str, Any]:
        last = messages[-1]
        if last.get("role") == "user" and "번호" in str(last.get("content", "")):
            retry_seen.append("호출됨")
            return _decision(content="재요청")
        if any(m.get("role") == "tool" for m in messages):
            return _decision(content="")
        return _decision(tool_calls=[_tc("get_fees", {})])

    retriever = FakeRetriever([])
    llm = _agent_llm(settings, decide, answer="이번 달 관리비는 확정 데이터 기준입니다.")
    events = await _run(llm, retriever)
    done = _done(events)

    assert not retry_seen, "도구 카드가 근거인데 불필요하게 재요청했다"
    assert done.status == "answered"
