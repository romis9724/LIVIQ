...
# ── notices: worker 예약 발행 스캔·전이 (H8-1, ADR-0015) ──────────────────
...
async def _insert_notice(
    ...
    *,
    ...
    result = await conn.execute(
        text(
            "INSERT INTO notices(tenant_id, title, body, status, audience, scheduled_at, "
            "deleted_at) VALUES(:t, 'x', 'y', :s, 'ALL', :sa, :da) RETURNING id"
        ).bindparams(
            t=tenant_id,
            s=status,
            sa=now if scheduled else None,
...
async def test_worker_scans_scheduled_notices_cross_tenant(
    owner_conn: AsyncConnection, seed: Seed
) -> None:
    """worker는 status=scheduled 미삭제 공지만 cross-tenant로 스캔(예약 발행 폴링)."""
    await _insert_notice(owner_conn, seed.a.tenant_id, status="scheduled", scheduled=True)
    await _insert_notice(owner_conn, seed.b.tenant_id, status="scheduled", scheduled=True)
    await _insert_notice(owner_conn, seed.a.tenant_id, status="published")  # 스캔 제외
    await _insert_notice(owner_conn, seed.a.tenant_id, status="draft")  # 스캔 제외
    await _insert_notice(
        owner_conn, seed.b.tenant_id, status="scheduled", scheduled=True, deleted=True
    )  # 삭제 → 제외

    await set_context(owner_conn, "liviq_worker", tenant_id=None)

    assert await _count(owner_conn, "notices") == 2, "worker가 scheduled 외 공지를 스캔함"


async def test_worker_notice_write_requires_tenant_context(
    owner_conn: AsyncConnection, seed: Seed
) -> None:
    """전이(UPDATE)는 표준 tenant_isolation — 올바른 tenant 컨텍스트에서만 성립(CRITICAL)."""
    notice_id = await _insert_notice(
        owner_conn, seed.a.tenant_id, status="scheduled", scheduled=True
    )

    # 잘못된 tenant(B) 컨텍스트: worker_scheduled_scan은 SELECT 전용 → UPDATE 대상 0행.
    await set_context(owner_conn, "liviq_worker", seed.b.tenant_id)
    wrong = await owner_conn.execute(
        text("UPDATE notices SET status = 'published' WHERE id = :i").bindparams(i=notice_id)
    )
    assert wrong.rowcount == 0, "타 tenant 컨텍스트로 공지 전이가 새어나감"

    # 올바른 tenant(A) 컨텍스트: 전이 성립.
    await set_context(owner_conn, "liviq_worker", seed.a.tenant_id)
    ok = await owner_conn.execute(
        text("UPDATE notices SET status = 'published' WHERE id = :i").bindparams(i=notice_id)
    )
    assert ok.rowcount == 1


async def test_worker_can_notify_under_tenant_context(
    owner_conn: AsyncConnection, seed: Seed
) -> None:
    """발행 알림: worker가 tenant 컨텍스트 하에서 users 조회·notifications 생성(GRANT 확인)."""
    await set_context(owner_conn, "liviq_worker", seed.a.tenant_id)

    assert await _count(owner_conn, "users") == 1  # 자기 단지 active 사용자
    inserted = await owner_conn.execute(
        text(
            "INSERT INTO notifications(tenant_id, user_id, type, title) "
            "VALUES(:t, :u, 'notice', 'x')"
        ).bindparams(t=seed.a.tenant_id, u=seed.a.user_id)
    )
