...
LLM: OpenAI-호환 단일 엔드포인트(Ollama·vLLM·OpenAI 등, env 교체) · **파일럿 확정 모델 llama3.1:8b**(H5-1 실측, [docs/09 §8.6](docs/09-implementation-harness.md) — dev·사내 서빙은 vLLM `qwen3-8b-awq`) · 임베딩 bge-m3(1024).
타입 공유: FastAPI OpenAPI → openapi-typescript 생성(`packages/api-types`).
인증: Redis 세션 + **자체 이메일+비밀번호**(Argon2id·검증 메일·`auth_tokens` — H7-1, [ADR-0014](docs/adr/0014-local-email-auth.md)) + 역할 가드. 웹은 세션 쿠키 1차(credentials CORS), dev 헤더(`X-Dev-*`)는 api의 local 보조(evals용).
배포 형상은 **둘** — 컨테이너 이미지 4종(api·ai-worker·web 2종) + `infra/compose.prod.yml` profiles 공용. ①3-tier VM + GHCR + GitHub Actions([ADR-0020](docs/adr/0020-container-deploy-3tier-vm.md)) ②사내 단일 호스트(Windows Server WSL2 Docker) + GitLab CI([ADR-0021](docs/adr/0021-gitlab-ci-single-host-wsl.md)).

## 구조 ([docs/02](docs/02-directory-structure.md) · 상세는 [ARCHITECTURE.md](ARCHITECTURE.md))

```text
apps/      web-resident · web-admin (Next.js) · api (FastAPI) · ai-worker (arq)
packages/  ui · config-ts · api-types (OpenAPI 생성) · ai-core · db (SQLAlchemy·Alembic)
infra/     docker-compose.yml(로컬 pg16+pgvector·redis·minio·neo4j) · compose.prod.yml · deploy-wsl.sh
mcp/       gmail·apt MCP 서버 · management_agent (Python)
evals/ tests/e2e/ scripts/                   # 측정 자산 · Playwright 여정 · 운영 스크립트
docs/ refs/                                  # 설계 문서 · 참조 자료
```

**구현 진척(어디까지 됐는지)의 단일 출처는 [CLAUDE.md](CLAUDE.md) 「구조」절 + [docs/09 §8](docs/09-implementation-harness.md)**이고,
디렉토리 상세는 [docs/02](docs/02-directory-structure.md)다 — 여기 사본을 만들지 말 것.
...
pnpm test        # turbo run test — vitest(web 2종+ui) + pytest(Python 4종)
pnpm db:migrate  # Alembic upgrade head (DATABASE_URL 필요)
pnpm e2e         # Playwright 여정 (infra 기동 필요)
pnpm generate:api-types   # FastAPI OpenAPI → packages/api-types 재생성
pnpm check:paths          # 컨텍스트 문서의 경로 참조 검증
```

> 없는 명령을 문서에 적지 말 것 — stale 참조는 없는 것보다 나쁘다. 루트 스크립트 정본은 `package.json`.
...
- Python은 FastAPI + Pydantic v2 + SQLAlchemy 2.0(async). Python 패키지 디렉토리에서 plain `uv run` 금지(형제 멤버 deps를 prune함) — `uv run --no-sync` 사용, 설치는 `uv sync --all-packages`.
- UI는 디자인 토큰만 사용(하드코딩 금지), 접근성 WCAG 2.2 AA, 라이트 테마 1차.
- 시크릿 하드코딩 금지. env는 부팅 시 검증(웹=Zod, Python=Pydantic Settings).

## 작업 방식

- **작업 사이클(H2부터)**: 작업 단위(Hx-y)마다 브랜치 → ①설계 갱신 커밋(구현 전 필수) → ②구현 커밋 → ③현행화 커밋(CLAUDE.md·ARCHITECTURE.md·docs/09 §8 상태) → ④PR(CI 그린+사용자 확인 후 머지). 상세: [docs/09 §3.1](docs/09-implementation-harness.md).
...
구현 [09](docs/09-implementation-harness.md) · 사업계획(역사적) [10](docs/10-project-plan.md) · 데이터 [11](docs/11-data-architecture.md) ·
배포 런북 [12](docs/12-deployment-runbook.md) · GitLab→WSL 배포 [13](docs/13-gitlab-wsl-deploy.md) · ADR [docs/adr/](docs/adr/README.md)
