...
- **동 스코프**: facility code가 동별(`EL-401-01`)이라 입주민 401동 → 401동 설비만. 세대
  스코프가 곧 동 스코프로 자연 성립(규칙 4).
- **배선 방식**: `plan_devices.facility_id`를 device_type×building 규칙으로 채우고
  (PG가 정본), 재색인이 Neo4j `SERVED_BY` 엣지로 반영. 상수 런타임 매핑을 버리고 데이터로
  고정 — 감사·재현 가능.

device_type → 설비 계통(code 접두) 대응(배선 규칙, 세대 기기가 없는 계통은 연결 안 함):

| 세대 기기(device_type) | 설비 계통(code 접두) |
|---|---|
| 화재감지기·소화기 | FR(소방) |
| 콘센트·분전함·월패드 | EP(전기)/수변전 |
| 온수·난방 배관 | WT(급수)·HX(열교환) |
| (동 공통) 승강기 | EL |
