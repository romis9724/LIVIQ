...
/** 단지별 가입 링크(?t=)로 계정 가입 → 검증 메일 안내 화면까지. */
...
test.describe.serial("가입 전 구간 여정 — 설치→단지→초대→명부→가입→승인 (H7-4)", () => {
  ...
  test("소장이 초대를 수락하고 직원 초대·명부 업로드를 한다", async ({ browser }) => {
    ...
    await expect(mgrPage).toHaveURL(/\/review-queue/); // MANAGER 첫 진입=검수 큐

    // 직원 초대(202) → 목록에 초대됨 행.
    await mgrPage.goto(`${ADMIN}/settings`);
  ...
  test("명부 불일치 주민은 대기 상태로 표시되고 일반 API가 차단된다", async ({ browser }) => {
    ...
    await signupResident(mismatch, journeyTenantId, JOURNEY.mismatchEmail, JOURNEY.password);
    await verifyAndLogin(
      mismatch,
      pg,
