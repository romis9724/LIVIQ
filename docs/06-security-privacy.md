...
- **인증**: **Google OAuth 2.0 + PKCE**(자체 비밀번호 미보관). OAuth 콜백은 **api(NestJS)**가 처리하며 `state`(CSRF)·PKCE 검증. Google **access/refresh 토큰은 저장하지 않는다**(로그인 식별용 `sub`만 사용). 최초 로그인 후 세대정보 입력 → `pending`, 소장 승인 시 `active`. `pending`/`rejected`/`inactive`/`withdrawn` 계정은 API 접근 차단(대기·거절 안내 화면만).
...
- **키 관리(결정 A)**: 앱 레벨 **봉투 암호화(envelope, AES-256-GCM)**. 부팅 시 **env 마스터 키(KEK)**를 **Zod 검증**(누락 시 기동 실패), 데이터 키(**DEK**)는 **per-tenant**로 KEK가 감싸 저장. 복호화·마스킹은 **전용 애플리케이션 서비스**만 수행 — DB 뷰·LLM 경로는 복호화 불가([03 §6](03-database-design.md)).
