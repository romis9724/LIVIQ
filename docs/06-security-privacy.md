...
> **세대 평면도 행 세부(H13-3~5, [03 §4.8](03-database-design.md))**: 입주민 열람은 소유권 검증(아래 §2)
> 그대로 — 본인 `household_id` 밖 세대의 `plan_devices`는 서버가 항상 거부한다. 관리자(MANAGER)는 전 세대
> 열람·편집(H13-4) 가능. 도면 이미지는 MinIO `{tenant_id}/` 프리픽스 + **서명 URL**(§5 파일 업로드 통제와
> 동일 계열). 어시스턴트 위치 질의 도구(H13-5)는 읽기 전용이며 조회 범위를 본인 세대로 강제한다.
...
- **AI 경로도 같은 인가를 받는다**: 어시스턴트 도구는 **스펙 노출 단계에서 역할로 필터**된다(`RESIDENT_ROLES`={RESIDENT} · `OFFICE_ROLES`={MANAGER,STAFF} · `FACILITY_ROLES`={FACILITY,MANAGER} — 권한 밖 도구는 모델에게 보이지도 않고, 보이더라도 실행 전 재검증). 세대·`tenant_id`는 **LLM 인자가 아니라 세션 컨텍스트**에서 온다(모델이 남의 세대를 지목할 표면 자체가 없다). 관리자 홈 AI 비서(`POST /admin/assistant/ask` — H20-2, [ADR-0028](adr/0028-admin-assistant-home.md))와 시설 도우미(`/admin/facilities/assistant`)는 **MANAGER 전용**(그 외 403).
...
- **`SYS_ADMIN` 허용 목록**(단지 콘텐츠 비열람 원칙의 구체화):
  - **접근 가능**: `tenants`(단지 메타), `jobs`, `outbox_events` 상태, 집계 메트릭(토큰/품질 대시보드), 공용 골든셋(`ai_eval_golden` `tenant_id` NULL), **플랫폼 AI 설정 `ai_backend_config`**(`/system/ai` — H15-1·H15-3, 전역 단일 행이라 단지 콘텐츠가 아니다. 저장된 `api_key`류는 어떤 응답에도 원문이 나가지 않는다 — 끝 4자 마스킹)만.
...
- **차량번호**(H9-5)는 PII: `parking_vehicles.plate_enc`에 **같은 봉투 암호화**(per-tenant DEK)로 저장 — 평문 컬럼 없음, 검색 해시 불요(세대 귀속 데이터라 번호판 조회 경로가 없다). 복호는 **관리자(MANAGER) 주차장 조회 API**만 수행하고, 주차 관리 목적상 **마스킹 없이 전량 표시**한다(관리자 화면 한정 — 그 조회는 `pii.plates_viewed`로 감사, §8). **번호판은 입주민 앱·LLM 경로에 나가지 않는다**(규칙 2) — 주차 데이터 자체는 두 경로에 열려 있으나 번호판은 **조회하지 않는다**: 입주민 주차맵(`GET /parking/map` — H17-2)은 점유 면 번호와 **본인 세대** 차량 위치만 내보내고(타 세대 번호판·동호수는 복호 경로가 없다), AI 주차 도구(빈자리·내 차 위치 — H15-4·H19-2)는 RESIDENT 전용 읽기 전용이며 `plate_enc`를 **SELECT조차 하지 않는다**(도구 결과는 그대로 프롬프트에 실리므로 애초에 꺼내오지 않는 것이 유일한 안전장치다). 세대 삭제 시 CASCADE로 함께 파기([03 §4.11](03-database-design.md)).

### 4.2 LLM 전송 차단 — 전 프로바이더 동일 (FR-AI-05, NFR-SEC-01)
- LLM/임베딩 호출 **직전** 마스킹/가명화 — 관문은 `ensure_masked` 하나다(`ai-core/masking`: 마스킹 후 잔존 PII 재스캔). 치환은 **결정적 플레이스홀더 `<PII:종류:n>`**(같은 원문=같은 토큰이라 답변 복원(`unmask`)이 성립): 주민번호·연락처·이메일·생년월일·동호수·계좌번호는 정규식으로, **이름은 호출자가 넘긴 목록**(로그인 사용자·대화 참여자)만 정확 치환한다 — 자유 텍스트 속 미지의 인명은 커버하지 못한다(NER 도입은 후속 과제).
...
- **클라이언트 IP**: `audit_logs.ip`는 프록시 뒤에서 신뢰 설정이 없으면 **프록시 IP로 찍힌다**(H11-1 실측). uvicorn은 `FORWARDED_ALLOW_IPS`(기본 `127.0.0.1`)에 속한 peer의 `X-Forwarded-For`만 신뢰하므로 배포에서 이 값을 프록시에 맞춰 지정한다([`infra/env.prod.example`](../infra/env.prod.example)·[12 §2](12-deployment-runbook.md)). Caddy가 XFF 끝에 실제 peer를 덧붙이고 uvicorn이 마지막 항목을 쓰므로 **프록시를 경유한 위조는 성립하지 않는다**(실측) — 위조 가능 조건은 api 직결이며, 그것은 §6.1 네트워크 경계가 막는다.
