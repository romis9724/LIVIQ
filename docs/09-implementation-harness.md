...
| H14. 시설관리 UX 개편·코드 체계 | 시설관리 탭 폐지→전체화면 그래프+플로팅 패널·시설 코드 체계(계통-위치-연번)·평면도 공통코드·트윈 UX 2건·주차장 3D | 코드 자동부여 유일성·tenant 격리(CRITICAL) + FloorPlan 잔존 노드 재발 방지 + 게이트 그린 + 시각 실측 | 🚧 진행 (§8.17) |
...
| 순서 | 작업 | 산출물 | 완료 기준 | 상태 |
|------|------|--------|-----------|------|
| H14-1 | 시설관리 한 화면 개편 | 탭 3종 폐지 → **전체화면 그래프** + 플로팅 패널(좌상단 현황 요약·우측 상세(기존 FacilityGraphPanel 재사용)·목록은 플로팅 패널 토글·평면도 편집은 도면 노드/목록 진입→**전체 오버레이**(기존 FloorPlanManager/Editor 재사용)) · 그래프에서 **PlanDevice 마커 제거**(include_plan 화면 소비 중단 — FloorPlan 노드는 **기본 표시**·토글 삭제) · AI 도우미 진입은 플로팅 버튼 · **FloorPlan 잔존 노드 정리+재발 방지**(seed_floor_plans가 (tenant, unit_type) 기준 기존 행 upsert — pg_id 보존) | 잔존 노드 0 실측 + 재시드 멱등 테스트 + 게이트 그린 + 시각 실측(1280·375px — 그래프 기본 도면 노드·플로팅 패널·평면도 오버레이·콘솔 0) | ⬜ |
