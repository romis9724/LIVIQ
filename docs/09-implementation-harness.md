...
| H15. LLM 백엔드 런타임 전환 | SYS_ADMIN AI 설정 UI(base URL·모델·API 키·연결 테스트)·`ai_backend_config` 전역 테이블·요청 단위 즉시 반영(env 폴백)·백엔드 비교 측정(ollama vs vLLM vs OpenAI) | SYS_ADMIN 인가(CRITICAL)·API 키 마스킹 응답 + 캐시 키 백엔드 분리 + 게이트 그린 + 시각 실측 | 🚧 진행 (§8.18 — H15-1 ✅·H15-3 ✅, H15-2 로컬 축 완료·GraphRAG 비교(G1~G4) 개발서버 실측 완료·OpenAI 축 대기 · H15-4 멀티-검색 라우팅+주차+Qwen3.5-9B 진행) |
...
| H15-4 | 멀티-검색 라우팅 + Qwen3.5-9B 후보 + 주차 최근접 | (인터뷰 확정 2026-07-31, 계획 `plans/resilient-hugging-ripple.md`) 복합 질의(그래프+벡터+구조화)를 LLM 자율로 멀티툴 라우팅. **P1** 주차 점유 PG 영속화([ADR-0023](adr/0023-parking-occupancy-persisted.md) — 프론트 시뮬 폐기)+RESIDENT 최근접 빈자리 도구 `find_nearest_available_parking`(공간 거리, 읽기 전용). **P2** 케이스에 복합·그래프·주차 추가 + `required_tools`(AND) 채점. **P2e** codex·cursor CLI 모델 무관 감사 후 케이스셋 동결. **P3** Qwen3.5-9B-AWQ(thinking-off) 배포 → 8B/9B/14B 동일세트 3자 측정(R28 — 라우팅·인용·지연·토큰·VRAM). **P4** 사전 승격 기준으로 파일럿 8B 유지/9B 채택 판정. 깃: GitLab 전용(GitHub 수동). OpenAI 축은 여전히 별도(키 대기) | 주차 tenant 격리·RLS·최근접 순수함수 단위테스트(CRITICAL) + 복합 PII 마스킹 게이트 + thinking 누출 스모크 + 게이트 그린 + 3자 측정표 | 🚧 진행 — **P1 완료**(점유 PG SoR·`parking_occupancy` 마이그레이션·최근접 도구 `find_nearest_available_parking`·관리자 뷰 전환·시뮬 은퇴 — ai-core 222·api 480·web 361·db 173 그린. 시각 실측은 dev 배포 후). P2~P4 대기 |
