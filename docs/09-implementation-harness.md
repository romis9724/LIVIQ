...
> 근거: 사용자 인터뷰 확정(2026-07-22). H8-1은 **별도 브랜치(feat/h8-1-notice-board)에서 병행 진행** — 본 표의 H8-1 행은 자리 예약이며 상세는 해당 브랜치의 ADR-0015·§8.10을 정본으로 머지 시 병합한다.
> 각 작업 단위는 §3.1 사이클(설계 갱신 → 구현 → 현행화 → PR)을 따르고, 머지는 단위별 사용자 확인 후 진행.

| 순서 | 작업 | 산출물 | 완료 기준 | 상태 |
|------|------|--------|-----------|------|
| H8-1 | 공지 게시판 전환 | (별도 브랜치 진행 중 — ADR-0015, AI 초안 제거·첨부·예약 발행) | 해당 브랜치 기준 | 진행 중 |
| H8-2 | 문서 게시판 전환 | [ADR-0016](adr/0016-document-board-versioned-attachment.md) — 관리자 전용 게시판(제목+본문(설명용)+첨부 1개 필수), `document_versions` 버전 이력(재업로드=version+1+재인제스트, 이력 다운로드만·롤백 없음), `document_chunks`→`content_chunks` 소스 일반화(notice 대비), soft delete+청크 즉시 삭제, 다운로드 API 경유 인가, 기존 데이터 폐기. web-admin 목록·작성·상세/수정·버전 이력 화면 | 인가·격리 테스트(**CRITICAL** — 교차 tenant 404·RESIDENT 거부), 재업로드→재인제스트→벡터 최신화 검증, 게이트 그린(pytest cov 80·vitest·E2E) | 진행 중 |
