...
| H8-4 | 공통 코드 관리 | `code_groups`·`codes` 스키마([03 §4.10](03-database-design.md)) + RLS·마이그레이션(단지 생성 시드+기존 단지 시드) + API 7종([01 §13](01-architecture.md) 설정·코드 관리) + 설정 메뉴(코드 관리 트리 — 그룹 선택→추가·수정·정렬·비활성·삭제) + 기본 코드 시드(NOTICE_CATEGORY·DOC_CATEGORY)([ADR-0017](adr/0017-tenant-code-registry.md)) | 코드 CRUD·순환 방지(parent_id)·is_system 그룹 보호(409)·STAFF 조회 전용 인가 테스트(**CRITICAL** — MANAGER 쓰기·STAFF 읽기 전용·교차 tenant 격리) + 게이트 그린 + 시각 실측 | 진행 중 |
| H8-5 | 동/호수 관리 | `buildings`·`households` CRUD + 층·호 범위 일괄 생성 + 설정 하위 화면. 입주민·명부 FK 연결 세대 삭제 거부(**CRITICAL**). 기존 시드 스크립트(`apps/api/scripts/seed_households_xlsx.py`)는 초기 대량 등록용으로 유지 | 삭제 거부 인가 테스트 + 일괄 생성 검증 + 게이트 그린 + 시각 실측 | 예정 |
| H8-6 | 공지·문서 코드 적용 | notices: `+category_code_id` FK(NOTICE_CATEGORY)·`event_start` date NULL·`event_end` date NULL(표시용 행사/작업 기간)·`target_buildings` JSONB NULL(동 id 배열, NULL=전체동, 표시용 — 알림 타게팅 백로그)·`keywords` text NULL(콤마 구분, H8-3 임베딩 텍스트에 포함). documents: `source_type`→`category_code_id` FK(DOC_CATEGORY) 전환 + 기존 데이터 라벨 매핑 마이그레이션. FK는 RESTRICT — 참조 중 코드 삭제 409 | 코드 참조 무결성 테스트(RESTRICT 삭제 거부) + 마이그레이션 라벨 매핑 검증 + 키워드 임베딩 반영 + 게이트 그린 | 예정 |
