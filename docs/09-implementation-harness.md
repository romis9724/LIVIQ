...
| H9-5 | 주차장 대시보드 (데이터 대시보드) | `parking_assignments` 마이그레이션(household_geometries **표준 RLS 복제** — tenant_isolation ENABLE+FORCE·`liviq_app` GRANT · **1행 = 배정 주차면 1개**, 위치·차량번호 선택) · `GET/POST/PATCH/DELETE /admin/parking` CRUD(**MANAGER 전용** · 세대(동/호)별 집계 + 행 목록 · 동/호 검색) · **차량번호 = 개인정보(규칙 2)** → 주차 테이블 `plate_enc`(bytea)·`plate_hash`(검색용 HMAC) 직접 저장(세대당 다건이라 per-person `pii_vault` 부적합 — `PiiCrypto` 암호화 서비스·per-tenant DEK만 재사용, 저장 시 암호화·조회 시 복호, **입주민 앱·LLM 미노출·관리자만 전체 표시**) · web-admin `/parking`(통계 타일 4종: 총 배정면·등록 차량·배정 세대·미배정 세대 + 세대별 표(동/호·배정면수·위치·차량번호·차량대수)·동/호 검색 + 세대 선택 CRUD 모달 — `FormField`·`Dialog` 재사용) · 메뉴 "주차장 대시보드"(🅿️ 트윈 대시보드 바로 아래 형제·MANAGER 항상 노출) · `lib/api.ts` 함수·`roles.ts` NavItem | tenant 격리·MANAGER 인가 테스트(**CRITICAL**) + plate 암호화 왕복 테스트(DB 암호문·복호 원문·타 tenant 404·plate 없는 행 허용) + FK 보호(household CASCADE)·CRUD 정합·세대별 집계 정합 테스트 + 게이트 그린(pytest cov·vitest·build·api-types drift) + 시각 실측(1280·375px) | 🔄 진행 중 (설계 — 본 커밋) |

> **백로그(수요 확인 후)**: 페르소나 부가정보 — ~~차량~~(**H9-5 주차장 대시보드로 채택**, 2026-07-25)·관계·직업(세대원 확장 테이블 별도 설계) · 트윈 집계 AI 읽기 도구(동/단지 통계만 — 개인 단위 금지, 규칙 2) · ~~VWorld 실사 3D~~(**H9-3로 채택**, 2026-07-24) · 설비-세대 정식 매핑 · 대시보드 액션 큐↔트윈 딥링크 · hover 하이라이트·지반고 샘플링.
