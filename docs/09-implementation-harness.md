...
| H2-6 | 검수 큐 | `needs_review` 메시지 큐 api([01 §13](01-architecture.md) 검수 큐 표) + messages 검수 필드 마이그레이션(reviewed_by·reviewed_at·review_note — [03 §4.3](03-database-design.md), H2-0 설계분) + web-admin review-queue 실연동(승인/반려·메모). **사후 검수** — 전달된 답변 회수 없음(정정 알림은 백로그), 골든셋 후보 축적 | 저신뢰 답변이 큐에 적재→승인/반려 처리 흐름 테스트 | ✅ 완료 (PR #7) → **H8-7에서 전량 제거**(라우터·화면·`messages` 검수 컬럼 drop `b2d9e4f7a1c3` — [ADR-0015](adr/0015-notice-board-replaces-ai-draft.md) 개정 노트). 남은 것은 `review_status=needs_review` 플래그뿐이고 저신뢰 답변은 담당자 연결 폴백 |
...
| H19-5 | 케이스셋 v2.1 + dev 재측정 | ①신규 시나리오 케이스 — `V2-QA-0331`~`0337` + `CX-R13`~`R15`(별도 세션에서 [`cases-draft.csv`](../evals/fixtures/chetmaeul-v2/cases-draft.csv)에 추가 완료) + H19-2·H19-3 도구 케이스. **선행: H19-4의 `V2-QA-0337` 라벨 전환**(폴백 → 평균 비교 답변 · 리졸버 추가 시 그것까지) — 낡은 라벨로 `gen`을 돌리면 구현된 기능이 실패로 채점된다 ②**선행: 공지 데모 시드의 dev 실행**(`notices_demo.py` 공지 86건은 `ebc08a7`로 main에 있으나 **시드 실행은 미완** — 데이터 반영이 사용자 판단이라 보류됐다. 시드 없이는 동 타게팅·8월 공지 케이스가 성립하지 않음) ③dev DB에서 `gen_labels.py gen` 재생성(로컬 gen은 문서·시설 UUID가 dev와 달라 라벨을 오염시킴 — R23 관례) ④3회 측정 후 MEASUREMENT-LOG에 회차 기록 | 신규 케이스 라벨 정합(리졸버 미해결 0) + hard_fail 0 + 라우팅률 R30 범위 내 + **인용률 비교는 3회 이상 중위로만**(CLAUDE.md 규율 — 단발 회차로 증감 판정 금지) + 측정 기록 커밋 | ✅ 완료 — ①draft 추가·②공지 시드·③dev gen 재생성(quality 337→349)·④측정 R31~R34 완료. 되묻기 도피(10.4%)를 R31에서 잡아 처방(97bd3b6) 후 R32에서 기준 회복(중위 6.7%). "라우팅률 R30 범위"는 세트 교체(89→135→147건)로 직접 비교 불가 — R32~R34가 새 기준선. hard_fail 전 회차 0. 단일 출처 MEASUREMENT-LOG R31~R34 |
| H19-6 | 답변 UX 정리(인용 마커·평면도 연동) | ①답변 끝 `[n]` 인용 마커를 **표시에서만** 제거(`markdown.ts` — 서버 텍스트에는 유지: 인용 검증·측정이 `[n]`을 본다. 숫자만 매치, 스트리밍 미완성 마커는 완성 후 제거) ②`find_in_floor_plan` 답변에 "평면도 보기" CTA → `/floor-plan?device=` 딥링크(도구 카드에서 기기 라벨 추출) | 마커 미표시 + 서버 인용 게이트 회귀 없음 + 게이트 그린 | ✅ 완료(912a104 — 행 누락분 소급 기록) |
| H19-7 | 민원 접수 CTA·복귀 동선 | (사용자 지적 — 접수 CTA가 다른 CTA와 이질적이고, 넘어간 화면에서 AI 비서로 돌아올 길이 없다) ①"민원 접수하기" CTA에 아이콘 추가(🅿️ 주차·🏠 평면도 관례와 통일) ②`/inquiries`를 어시스턴트 딥링크(`?compose=1`)로 열면 **상단 뒤로가기** 표시(`router.back()` — [FloorPlanView](../apps/web-resident/src/features/floor-plan/FloorPlanView.tsx)·ParkingMapView 관례) → AI 비서 대화로 복귀(대화는 sessionStorage 복원 — H17 UI 정리) ③하단 "민원" 탭으로 직접 진입하면 뒤로가기 **비노출**(딥링크 param 유무로 판정, 마운트 시 1회 — 프리필과 동일 규칙) | compose 딥링크에서만 뒤로가기 렌더(탭 진입 미렌더 테스트) + 프리필·IME 회귀 없음 + 게이트 그린 + 시각 검증(스크린샷) | ✅ 완료 — 판정은 마운트 시점 `prefill.isCompose` 캡처(프리필은 목록 복귀 시 1회성 폐기라 직접 못 씀). 새 순수 로직 없음 — 딥링크 판정은 `prefill.test.ts` 기존 커버, 렌더 테스트 인프라(testing-library) 미도입 유지. 시각 검증 4종 통과(탭 진입 미표시·딥링크 표시+프리필·back 복귀·콘솔 0) |
...
| H19-10 | 복원 대화 스크롤 즉시 점프 | (사용자 지시) AI 비서 진입 시 이전 대화가 있으면 스크롤을 애니메이션 없이 **즉시** 맨 아래로 — 첫 표시만 `behavior:"auto"`, 이후 추가 메시지는 smooth 유지 | 시각 검증(복원 진입 시 최신 메시지 즉시 노출) + 게이트 그린 | ✅ 완료 — `AssistantChat.tsx` `hasScrolledRef` 분기(PR #135) |
| H19-11 | 관리비 질의 날짜/null 결함 수정 | (사용자 보고 "관리비가 안나와" 근본 원인) ①모델이 오늘 날짜를 모름 → `agent_system_prompt(today)`로 KST 오늘 주입, 연도 없는 시점 표현("이번 달"·"8월")의 기준 명시 ②모델이 생략 인자를 문자열 `"null"`로 보냄 → `GetFeesArgs.period`에 null 리터럴 폴딩 validator | get_fees 공회전 소멸(로컬 6/6) + dev 판정은 R34로 | ✅ 완료 — 커밋 91ab048(#136), dev 판정 R34: pass 중위 67→83/147 범위 비겹침, 관리비 멀티턴 0/4→2/4. R23의 "상대날짜 매핑 실패 22%"의 뿌리였음 |
...
> **H20-1(관리비 동·단지 평균 조회)은 병렬 브랜치**(`h20-1/fees-scope-average`)가 선점한 번호 — 이 절의 기점 단위는 H20-2이고, H20-1 행도 이 표에 함께 둔다.

| ID | 작업 단위 | 범위 | 완료 기준 | 상태 |
|----|-----------|------|-----------|------|
| H20-1 | 관리비 동·단지 평균 조회 | (사용자 요구 — "402동 관리비는?"=동 평균, "전체 관리비는?"=단지 평균, 세부내역도 평균으로. 본인 항목 질의는 현행 유지) ①`get_fees`에 `scope` 인자 추가(새 도구 없음 — ADR-0026 결정 3의 "도구 수=8B 라우팅 예산" 유지): 미지정=본인 세대(현행 불변), `"402동"`→해당 동 평균, `"전체"`→단지 평균. validator가 표기 정규화("402"→"402동", "단지"/"모든 동"→전체). LLM이 동을 지정하는 것은 ADR-0026 결정 1(소유권 값 주입)과 충돌하지 않음 — 여기서 동은 격리·소유권 값이 아니라 **공개 집계 차원**이고 개별 세대 값은 어떤 경로로도 안 나감 ②평균은 총액+**항목별(breakdown name·level별)** 전부 SQL 집계(`jsonb_array_elements` unnest 후 avg — H19-4와 동일 계열, AI 재계산 0) ③표본 하한 k=10 재사용 — 미달 시 평균 거부, 존재하지 않는 동은 "해당 동 없음"(규칙 1 — 추측 금지) ④quote(LLM 경로)+`data`(카드, `fee_table`에 `scope{kind,label,sample_size}` additive 키) 양쪽 반영, 프론트 `structured.ts`+`FeeTable` 스코프 라벨 렌더 ⑤골든셋 — `fees-avg:<dong\|complex>:<period>` 리졸버 신설 + draft 케이스(동·전체·항목 평균), dev 라벨·측정은 후속 ⑥다중 월×평균 조합은 범위 외(단일 월, 기본 최신) | 개별 세대 금액 미노출(CRITICAL) + 표본 하한 미달 시 평균 거부 + tenant 격리(`:tid`+RLS, 집계는 같은 단지 안) + 수치는 확정 데이터 SQL 집계 그대로(규칙 5) + 본인 세대 경로 회귀 없음 + 게이트 그린 | ✅ 완료 — PR #151(누락됐다 화면 검증에서 발견·머지, 케이스 0354~0357로 재번호 — H20-2와 충돌), 후속 2건: #152 본인 세부 항목(전기료 등)을 quote에 노출(상위 3항목만 노출돼 항목 질의가 no_evidence였던 결함), #153 scope 본인 동의어 폴딩(모델이 본인 질문에 scope="본인 세대"를 채움 — validator가 접음). dev 화면 3종 실측 통과: 402동 평균(표본 58세대)·단지 전체 평균(322세대)·본인 전기료(확정값 그대로, 규칙 5). 후속 3건(사용자 버그 신고 — "아파트 평균에 우리집 혼입, 두 값 비교 불가"): #155 `compare_fees` 공통 비교 도구(targets 쉼표 2~4 — 본인=실값·동/전체=SQL 집계, `fee_compare` 카드, 공용 조각 `fees_common.py` 분리로 get_fees와 같은 SQL·표본 하한 공유), #156 단독 "아파트"·1인칭(나/저/우리) scope 동의어 폴딩, #157 get_fees 설명 보강("아파트 관리비"=scope 전체 — 8B가 scope를 안 채우던 실측 잔여). dev 화면 재검증: "아파트 관리비는?" 단지 평균 카드 ✅ + 비교 3형(본인 vs 전체 +8,390원 · 401동 vs 402동 · 402동 전기료 vs 전체 평균) ✅ |
| H20-2 | 관리자 AI 비서 홈 | ①api: `POST /admin/assistant/ask`(`require_roles("MANAGER")`·`channel="admin"`·`_assistant_response` 재사용·답변 캐시 우회 — ADR-0028 결정 2) ②@liviq/ui: 어시스턴트 코어 승격(SSE 파서·스트림 훅·세션 저장·마크다운·StructuredBlock·진행 단계·출처 묶기 — 조립·CTA는 앱별 유지, ADR-0028 결정 4) + web-resident 재배선(동작 불변) ③web-admin: `/assistant` 신설(좌 채팅+우 민원현황 패널 — 상태 카운트·최근 목록) · `/dashboard`→`/inquiry-status` 개명·관리소 운영 그룹 이동·AI 도우미 현황 섹션 삭제 · `roleHome` MANAGER→`/assistant` · 진입 브리핑(빈 대화 자동 질의·말풍선 숨김·날짜 포함) | RESIDENT의 `/admin/assistant/ask` 403(CRITICAL) + admin 채널 캐시 미사용(resident 캐시 미혼입 테스트) + web-resident 회귀 없음(기존 테스트 그린) + 브리핑 실패 시 폴백(화면 차단 금지) + OpenAPI 재생성 + 게이트 그린 + 시각 검증 | ✅ 완료 — dev 시각 검증 전 항목 통과(로그인→/assistant 자동 진입·브리핑 인사+요약+할 일·CTA·말풍선 숨김·패널 카운트=DB 일치·멀티턴 "미배정 6건" 정답·새 대화 후 재브리핑 없음·리로드 복원·≤1024px 스택·콘솔 0). 함정 2: ①StrictMode effect→cleanup→effect에서 마운트 즉시 ask는 abort로 죽고 ref 가드가 재발동까지 막아 "검색 중"이 영구 잔류 — **타이머 예약+cleanup 취소**로 해소 ②브리핑 발동 판정을 훅 messages로 하면 복원(effect)과 레이스 — **탭 저장소 동기 조회**로 판정. 브리핑 문구는 8B 라우팅에 민감(변형 2종 실측 기각: "로그인했습니다" 삽입→문서 검색 폴백, 인사 지시 축약→인사 누락) — 문구 변경 시 재실측 필수, 꼬리 변경은 저장본 숨김 판정도 깨뜨림. RESIDENT 403은 실브라우저에서도 확인(잔존 입주민 세션이 정확히 403). 라우팅 반복 측정은 후속 — draft V2-QA-0350~0353 추가 완료(브리핑 원문·직접 질의·멀티턴 후속·변형 표현, `inquiry-summary:` 리졸버는 dev gen 시 신설 판정). **측정 결착(R38→R39)**: R38이 브리핑 답변이 "오늘 민원 없음"으로 비는 `days=1` 오독을 보고했으나(라우팅·인용·hard_fail은 전부 통과 — 자동 게이트가 못 잡는 결함), 실체는 **골든셋 케이스가 구 문구를 들고 있던 것**이었다. 실제 꼬리는 044d0c0(H20-3)에서 이미 "**최근 7일** 민원 현황"으로 기간을 선제 명시하도록 바뀌어 있었다 — R38은 화면에 없는 문구를 측정했다. 케이스 turn_1을 `briefingPrompt()` 현재 출력과 동기(6a03e6a)한 뒤 R39 3회 재실측: 라우팅 중위 100%·hard_fail 0·0350 quote가 7일 창 집계(6건 = 미배정 2·처리중 2·완료 2)로 라벨 일치. **도구 스펙(`SummarizeInquiriesArgs.days`)은 변경하지 않았다**(R38 처방 ⓑ 기각 — 프론트 문구로 이미 해소). 기각 이력에 추가할 함정: 이 자리의 케이스는 문구의 **사본**이라 `briefingPrompt()`를 고치면 CSV(cases-draft·quality-cases-v2)도 같이 갱신해야 하고, 안 하면 게이트는 초록인 채 유령 결함이 보고된다 |
| H20-3 | 관리자 대화 복원(당일) | (사용자 요구 2026-08-02 — "이전 대화 기억 + 일자 바뀌면 새 대화 + 기본 기능은 입주민과 동일", ADR-0028 결정 2 개정) ①api: `GET /admin/assistant/conversations/latest`(MANAGER·channel=admin) — 기존 resident latest와 공유 헬퍼로 통합하되 **마지막 메시지의 KST 날짜가 오늘일 때만** 대화 반환(아니면 빈 응답 — 새 날=새 대화) ②@liviq/ui `useAssistantStream`에 `restored` 플래그 additive(복원 경로 종료 신호 — resident 렌더 계약 불변) ③web-admin: 탭 저장 키에 KST 날짜 접미(`liviq.admin.assistant.thread.v1.<YYYY-MM-DD>`) + `fetchLatest` 배선 + 브리핑 발동을 "저장·서버 복원 모두 빈 뒤"로 이동(현행 저장소-단독 판정 대체) | 당일 대화 복원(재로그인·새 탭) + 어제 대화 미복원(백데이트 테스트) + RESIDENT 403(CRITICAL) + resident latest 회귀 없음 + 브리핑: 복원 시 미발동·빈 복원 시 1회 발동 + OpenAPI 재생성 + 게이트 그린 + 시각 검증 | ✅ 완료 — dev 시각 검증: 재로그인 시 오늘 admin 대화 서버 복원(브리핑 미발동·자동 질의 말풍선 숨김 유지) · DB 백데이트로 어제 대화 만들면 미복원 + 새 브리핑 발동 · 콘솔 0. restored 플래그는 messages와 같은 태스크에서 커밋(먼저 true가 되면 복원 대화 위에 브리핑이 낌 — 훅 테스트로 고정). 알려진 UX: 시설 도우미도 channel=admin이라 그 당일 대화가 홈에 복원될 수 있음(격리 문제 아님 — 필요 시 채널 분리 후속). 사내 배포 실화면 보고 2건 반영: ①채팅 헤더 L 마크 삭제(AdminShell 로고와 중복 — 입주민 헤더는 불변) ②브리핑 되묻기 도피(배포 8B가 기간을 되물음) — `AskRequest.allow_clarify`(additive, 기본 true)로 **자동 발화만 되묻기 차단** + 꼬리에 "최근 7일" 명시(레거시 꼬리 병행 숨김, 라우팅 유지 실측 1회) |
| H20-4 | 답변 인라인 출처 배지 | (사용자 요구 2026-08-03 — "답변 안에 출처 아이콘, 클릭·호버 시 툴팁") ①`assistant-markdown`에 `answerSegmentBlocks` 신설 — 본문 `[n]` 마커를 벗기는 대신 텍스트 조각별 `refs`로 추출(`answerBlocks`는 파생으로 재정의 — 표시 텍스트 불변, 기존 소비처 무회귀) ②`CitationMark` 컴포넌트 — 번호 배지 버튼, 호버·키보드 포커스는 CSS·클릭(모바일)은 토글로 툴팁(문서 제목·조항/페이지·인용문 4줄 클램프) ③`AnswerBody`에 `citations` 옵션 — 대응 citation 없는 번호(스트리밍 초반·모델 오기입)는 기존대로 표시에서 제거, 서버 계약 변경 없음(SSE citation이 이미 제목·조항·quote 보유) ④입주민·관리자 두 앱 답변·스트리밍 경로 배선 | 마커 파싱 파생 관계 테스트(segment↔기존 텍스트 일치) + 배지·툴팁 렌더/토글/무-citation 폴백 테스트 + XSS 없음(React 엘리먼트만) + 게이트 그린 + 시각 검증 | ✅ 완료 — 로컬 화면 실측: 관리비 답변 본문 "…176,601원입니다.①" 배지 렌더·클릭 시 툴팁(제목+인용문 클램프)·blur 닫힘. 도구 인용(quote가 도구 텍스트 전체)은 4줄 클램프가 방어 |
| H20-5 | 주차맵·최근접 도구 3건 | (사용자 신고 2026-08-03) ①`find_nearest_available_parking` — 세대에 전기차가 있으면 일반 질의에도 전기차 면 포함(317면 28m가 더 먼 일반 면보다 가까운데 누락 — 거리 계산은 정상, `_is_free` 타입 필터가 원인. 8B는 "충전" 언급 없이 `ev_preferred`를 안 채우므로 `parking_vehicles.is_ev` 데이터로 판정 — `bool_or` 본인 세대 스코프) ②`ParkingMap` 확대 클릭 버그 — pointerdown 즉시 `setPointerCapture`가 click을 뷰포트로 리타게팅해 SVG 면 클릭 위임이 죽던 결함, 드래그 임계(4px) 초과 시에만 캡처 ③면 상세 패널 — 종류·상태(내 차는 입차 경과)·AI 추천 순위·우리 동 승강기까지 거리(`GET /parking/map`에 `my_dong` additive + layout `cores` 프론트 파싱, `spot-detail.ts` 순수 함수) ④초기 줌 3×→5× | EV 세대 포함/미보유 제외 도구 테스트 + 면 상세 순수 함수 테스트(거리 축척 13px/m = ai-core PX_TO_M 동일) + 타 세대 PII 무노출 회귀(`my_dong`은 본인 동 — 키 집합 고정 테스트 갱신) + OpenAPI 재생성 + 게이트 그린 + 시각 검증 | ✅ 완료 — 로컬 실측: 5× 진입·확대 상태 면 클릭 선택·상세 패널(013면 3순위 29m = 도구 거리와 일치)·드래그 팬 회귀 없음 |
| H20-6 | 본인 관리비 답변에 범위 평균 카드 혼입 억제 | (사용자 신고 2026-08-03 — 본인 관리비를 물었는데 단지 평균 카드가 같이 떴다) 원인은 라우팅이다: 본인 관리비 질의에 8B가 `get_fees(scope=전체)`를 덤으로 한 번 더 호출해 본인 카드와 범위 평균 카드가 한 답변에 실렸다. ①`packages/ui/src/components/assistant/assistant-structured.ts`의 `structuredBlocks()`에서 억제 — 같은 답변에 `scope` 없는(=본인) `fee_table`이 있으면 `scope` 있는 `fee_table`은 숨긴다 ②`fee_compare`(월 간 비교) 카드는 필터 대상이 아니라 본인 카드와 공존한다 ③동·전체 단독 질의(scope 카드만 있는 답변)는 그대로 노출. 서버·도구는 무변경 — 도구 설명·라우팅에 손대면 H20-1의 범위 조회가 함께 흔들린다 | `assistant-structured.test.ts` 단위 3건(혼재 시 본인 카드만·scope 단독은 노출·본인+`fee_compare` 둘 다 노출) + 게이트 그린 | ✅ 완료 — 프론트 억제로 마감(서버·도구 무변경). 실측 번호 없음 — 단위 테스트로 고정 |
| H20-7 | 시설 생활 동의어 — 검색 질의 확장 | (사용자 요구 2026-08-03 — "두꺼비집=분전함, 동의어 조사해 답변 품질 향상" = 시나리오 RES-3 동의어 처리) ①`ai_core.synonyms.FACILITY_SYNONYMS` 10항목 — 표준어는 전부 실데이터 대조로 선정(승강기 122 vs 엘리베이터 29 등 코퍼스 카운트), 기각 근거 명시(수도꼭지→수전은 이 단지 `수전설비`가 **전기** 수전이라 오염, 전구는 "전구역" 부분일치) ②`expand_query` — 질의 원문 뒤 괄호 병기(치환 아님·원문 보존), **임베딩 텍스트 한정**(프롬프트·히스토리·도구 설명 무수정 — 8B 라우팅 민감) ③적용은 질의측 embed 2곳만(`search_documents`·`search_facility_graph`) — 색인측(ingest·graph_sync)은 미변경 ④평면도 매칭(두꺼비집→분전함)은 `floor_plan_parser.ELEMENT_SYNONYMS`가 **이미** 접고 있어 미변경 | 사전 매치/비매치/중복 스킵/부분일치 오탐 가드 테스트 + embed 페이로드 캡처 테스트(확장 반영 확인) + 게이트 그린(402 passed·cov 96%) | ✅ 코드 완료 — 검색 회수율 개선치는 dev 골든셋 재측정 몫(인용률 비교는 3회 중위 규칙). `search_similar_inquiries`(pg_trgm)는 매칭 방식이 달라 미적용(별건) |
| H20-8 | 입주민 주차 3D — 추천 순위 비콘 | (사용자 요구 2026-08-03 — "3D 버튼, 추천 3곳 순위 비콘(1,2,3) + 카메라 이동" = 시나리오 RES-2 화면 연동) ①`ParkingScene3D`·`parking-scene-data`를 @liviq/ui 공용 승격(admin H14-4 → 두 번째 소비자) — 점유는 `SceneOccupant(external·group)`로 일반화(입주민은 소속 없는 익명 점유, 규칙 2), `matchesGroup`·`EXTERNAL_GROUP`·`PX_TO_M` 공용화 ②`setBeacons` — 추천 면 위 역원뿔+순위 스프라이트, 부유 애니메이션(reduced-motion 정지), `--pk3d-beacon` ③입주민 `/parking` 2D/3D 토글(dynamic ssr:false) — 진입 부감→1순위 카메라 이동, 순위 칩·전체 보기·면 클릭 선택은 H20-5 상세 패널과 동기 ④3D 팔레트 `:root`를 ui `parking-scene-3d.css`로 이동·`isWebglSupported` 승격 | 씬 데이터 순수 함수 테스트 이관(20) + 두 앱 게이트 그린 + admin 3D 회귀 시각 확인 + 입주민 3D 시각 검증(비콘·카메라·칩·상세 동기) | ✅ 완료 — 로컬 실측: 3D 토글→부감→1순위 217면 클로즈업+보라 비콘 "1"·순위 칩 이동·전체 보기 복귀·상세 패널 동기(317면 전기차 2순위)·admin 3D 무회귀. ADM-1(장기주차 비콘)·RES-1(내 차 클로즈업) 화면 연동은 이 씬 재사용 후속 |
| H20-9 | 주차 3D 딥링크 — ADM-1·RES-1 화면 연동 | (시나리오 화면 연동 잔여 2건) ①`SpotBeacon` 일반화 — rank → `label`+`color`(추천 보라 숫자 · 내 차 파랑 "내 차" · 장기주차 주황 순번) ②입주민: 어시스턴트 "주차위치 보기" CTA가 `?view=3d`로 3D 진입(RES-1·RES-2) — 내 차 면은 파란 "내 차" 비콘+클로즈업, 상세 패널 "추천 순위" 줄은 내 차 면에서 생략 ③관리자: 장기주차 답변 CTA "주차장 3D에서 보기"(ADM-1) — 면 번호는 도구 카드 quote 확정값에서만 추출(`assistant-links.ts`, 입주민 links와 같은 구조), 딥링크 진입 시 주황 순번 비콘(오래된 순)+첫 면 선택→카메라 이동 ④URL 파라미터는 전부 검증(면 번호 패턴·view 화이트리스트) | links 추출·href·파라미터 검증 테스트(입주민 16·관리자 5) + 게이트 그린 + 두 앱 시각 검증 | ✅ 완료 — 로컬 실측: 입주민 `?spot=208&view=3d` → 3D 자동 진입·파란 "내 차" 비콘·클로즈업·상세 패널("57시간 전 입차·승강기 18m") / 관리자 `?spot=321,378,377&view=3d` → 주황 비콘 1·2·3·321면 카메라 이동·상태바(외부 차량·117시간 경과). CTA 라우팅 실측(LLM 경유)은 dev 시나리오 검증(H20-10)에서 |
| H20-10 | 시나리오 6종 dev 시드 SQL 기록 | 시나리오 실측에 쓴 dev 데이터를 파일로 남긴다 — `evals/fixtures/seed-scenarios-dev.sql`(21줄) 신설. ①ADM-2용 404동 민원 7건(소음 3·주차 2·설비 1·지난주 소음 1 — 미처리 `received` 2건 포함) ②ADM-3용 402동 승강기 `EL-402-01`의 `next_check_at`을 2026-08-10으로 UPDATE + `maintenance_logs` 2건 + `incidents` 1건 ③주석에 함정 둘을 명시 — UUID가 dev 실측값이라 재실행하면 중복 삽입된다는 것, 직접 INSERT는 outbox 를 만들지 않으므로 Neo4j 반영이 필요하면 `outbox_events`를 따로 넣어야 한다는 것 | SQL 파일 기록(테스트·게이트 대상 아님) | ✅ 시드 SQL 기록 완료 — dev 실행·시나리오 실측은 별건 |
| H20-11 | 입주민 UX 4건 | (사용자 지시 2026-08-03) ①홈 하단 바로가기 아이콘 그리드 삭제(`apps/web-resident/src/features/home/HomeView.tsx`·`home.css`) — 하단 탭·관리비 카드·AI 비서 CTA로 전부 도달 가능해 중복이다 ②`AssistantChat.tsx`의 `FALLBACK_TEXT.no_evidence`를 대화체로 교체(닫힌 채 미머지였던 PR #140을 사용자 지적으로 회수) ③`ParkingView3DResident.tsx` — 3D 순위 칩 2·3 을 눌러도 1순위로 되날아가던 버그: 인라인 `map`이 렌더마다 새 배열을 만들어 비콘 effect 가 재발화한 것이 원인 → `introFlownRef`(씬당 인트로 비행 1회) + `myVehicleSpotNos` 메모이즈 ④`packages/ui/.../useAssistantStream.ts` — 재로그인 복원 후 말풍선이 뒤섞이던 React key 충돌: 저장은 꼬리 `slice(-40)`인데 `seq`를 메시지 **개수**로 되살려, id 번호가 개수보다 큰 대화에서 새로 발급한 id 가 저장된 id 와 충돌했다 → `maxMessageSeq()`(id 최댓값으로 seq 복원) | `useAssistantStream.test.tsx` 복원 id 충돌 테스트 + 게이트 그린 | ✅ 완료 — ④만 단위 테스트로 고정, ①~③은 화면 수정(실측 번호 없음) |
| H20-12 | 관리자 AI비서 CTA 패리티 | (사용자 지적 2026-08-03 — "관리자 비서에 '주차위치 보기'·'평면도 보기' 같은 기능이 없다") 입주민 전용 도구(`find_available_parking`·`get_my_vehicle_location`·`find_home_devices`)는 `RESIDENT_ROLES` 게이트라 관리자 답변에 **라우팅 자체가 불가** → 같은 CTA 대신 관리자가 실제로 받는 도구에 맞는 **등가 CTA**를 단다. ①시설 도구 3종(`get_facilities`·`get_overdue_checks`·`search_facility_graph`) 라우팅 답변에 "🔧 시설관리에서 보기" → `/facilities` ②`get_recent_notices` 답변에 "📢 공지사항 열기" → `/notices` ③폴백 문구를 입주민(#140 반영본) 대화체로 정렬 — 내용은 관리자 맥락 유지(담당자 연결이 아니라 관리자가 직접 볼 화면 안내). 판정은 `toolPath`만 보고(모델 자유텍스트 파싱 금지, 규칙 8), 딥링크 파라미터는 두지 않는다 — `/facilities`는 전체화면 3D 그래프 하나라 강조할 쿼리 계약이 없다(설비 코드 강조가 필요해지면 그때 additive) | 도구 판정 순수 함수 테스트 + 관리자 게이트 그린 + 시각 확인 | ✅ 완료 — 로컬 API 실측(dev 헤더·8B): "승강기 몇 대 있나요?"→`get_facilities` answered, "최근 공지사항 알려주세요"→`get_recent_notices` answered(5건 목록) = 두 CTA 모두 실제로 뜨는 경로. "점검 기한이 지난 설비"는 `get_overdue_checks` 라우팅되나 로컬 데이터가 없어 fallback(=CTA 미표시, 의도대로). 브라우저 시각 확인은 기존 CTA 2종과 동일 마크업이라 생략 |
| H20-13 | 민원현황 패널 드릴다운 | (사용자 요구 2026-08-03 — "상태 카드 클릭 시 목록, 미배정·처리중·재확인은 접수일자로 재촉, 완료는 완료일자, 민원 클릭 시 대화·처리 내역 요약") ①**서버 변경 없음** — 관리자 목록은 `GET /admin/inquiries`(status 필터 기존), 처리 내역은 `GET /inquiries/{id}/events`(관리자 열람 이미 허용). 패널이 이미 전량을 1회 로드하므로 카드 클릭은 **클라이언트 필터**(추가 왕복 0) ②완료일자 전용 컬럼 없음 → `updated_at` 사용(완료 후에는 `_guard_not_done`으로 갱신 경로가 잠겨 사실상 완료 시각, 정확한 시각은 타임라인) — 프론트 `Inquiry.updatedAt` 매핑만 추가(서버 응답에 이미 존재, OpenAPI 무변경) ③미완료(미배정·배정됨·처리중·재확인)는 접수일자+`N일 경과` 배지·**접수 오래된 순**, 완료는 완료일자·**최신순** ④민원 클릭 → 인라인 확장: 민원 내용 발췌 + 처리 내역 압축 목록(events 시간순, 기존 `inquiry-admin/data.ts` 헬퍼 재사용). **민원↔대화 연결 필드가 스키마에 없어**(conversations·messages에 inquiry_id 없음) "AI 대화 요약"은 민원 본문 발췌로 폴백하고 LLM 요약 엔드포인트는 신설하지 않음(규칙 7) ⑤행의 "민원 관리에서 열기"는 `/inquiries?inquiry=<id>` 딥링크(InquiryAdmin에 `useSearchParams` 선택 배선 + Suspense 경계) | 경과일·정렬·요약 순수 함수 테스트(오늘 기준 N일·완료건 완료일 표기·미완료 오래된 순) + 서버 계약 무변경(OpenAPI 재생성 불필요) + 패널 로드 실패 시 채팅 무영향 유지 + 게이트 그린 + 시각 검증 | ✅ 구현 완료 — 순수 함수 15 + 패널 상호작용 4(카드 클릭 시 재조회 0·펼치기 전 events 미조회·조회 실패해도 패널 생존) 그린, 게이트 그린(lint·typecheck·test 389·build). 정적 스크린샷 검증에서 **기존 결함 발견·수정**: 전역 list/button reset 이 없어 패널 `ul`이 기본 마커·좌여백을 노출하고 있었다(H20-2 잔여) — 패널 목록 4종 `list-style:none` + 카드·줄 버튼 겉모습 상속으로 정리. 실서버 시각 검증(로그인→카드 클릭→민원 펼침)은 dev 확인 몫. **AI 대화 요약은 폴백 상태** — 민원↔대화 연결 필드가 없어 접수 본문을 보여준다. 진짜 대화 요약이 필요하면 `inquiries.conversation_id`(접수 CTA가 채움) 신설이 선행(후속 후보) |
| H20-14 | 문서 제목 NFC 정규화 | (사용자 신고 2026-08-03 — dev 업로드 문서 "첫마을4단지 관리규약"이 화면 검색·SQL LIKE 모두 불일치, 11자로 보이는데 실제 25자) 원인은 zero-width 혼입이 아니라 **macOS 파일명의 NFD(한글 자모 분해)** — 업로드 폼이 `file.name`을 제목 기본값으로 넣고 서버가 그대로 저장했다(자모는 category Lo라 보이지 않는 문자 필터로는 안 걸린다). ①경계 정규화 `clean_title`(`app/schemas/documents.py`) — NFC 합성 + Cc/Cf 제거(zero-width·BOM·word joiner, 개행·탭은 남겨 공백 접기로 흡수) + 연속 공백 접기 + trim, 정규화 후 빈 문자열이면 422 ②`DocumentTitle` Annotated 타입 하나를 제목 저장 경로 **양쪽**(작성 Form·PATCH)에 적용 — OpenAPI 무변경 ③프론트 `titleFromFilename`은 입력창 기본값만 정리(정본은 서버) ④기존 오염 데이터 마이그레이션 없음(dev 1건 수동 교정 완료·운영 없음) | NFD 제목 저장 후 `q` 검색 매칭 + zero-width·연속 공백 정리 + 보이지 않는 문자만 있는 제목 422(작성·수정 양쪽) + 정상 제목 무변형 + 게이트 그린 | ✅ 완료 — api 530 passed(cov 96%)·mypy 그린·web-admin 395 passed·OpenAPI 드리프트 0 |
| H20-15 | 주차 3D 카메라 해제·비콘 축소 | (사용자 신고 2026-08-03 — "'주차장 3D에서 보기' 클로즈업 뒤 카메라 고정 해제해 달라, 다른 위치로 이동이 안 된다 / 핀이 너무 크다", 양 앱 동일) 실측으로 **OrbitControls 자체는 잠기지 않았다**(클로즈업 후 회전·줌·팬 모두 좌표 변화 확인) — 고정처럼 느껴진 실체는 둘. ①**짧은 클릭이 매번 재비행**: 캔버스 클릭(6px·400ms 이내)이 면 선택으로 잡히고 소비자의 `selectedNo` 효과가 `flyToSpot`을 다시 걸어, 조작하려 누를 때마다 클로즈업으로 끌려갔다 → 씬이 자기가 픽한 면을 기억해 되돌아온 fly 한 번을 무시(`selfPickedNo`, 공용 씬 1곳 수정 = 양 앱 해결). ②**비행 중 입력 무시**: 트윈 동안 렌더 루프가 `controls.update()`를 건너뛰어 1.8초간 드래그가 씹혔다 → pointerdown·wheel 이 진행 중 fly-to 를 즉시 해제(`releaseCamera`). ③이동(팬)은 오른쪽 끌기·두 손가락이라 안내 문구에 명시. ④비콘 절반 축소 — 원뿔 r1.6→0.8·h3.2→1.6, 라벨 스케일 1/2, 부유 진폭 0.7→0.35, 기준 높이 6.5→4.6m | 브라우저 실측(관리자·입주민 각각 딥링크 진입 → 클로즈업 완료 후 드래그로 카메라 각도 변경·클릭해도 안 끌려감·비행 중 드래그가 fly 중단) + 부감에서 비콘 식별 유지 + 게이트 그린 | ✅ 완료 — 로컬 실측(3001 dev): 클로즈업 후 회전 pos(97.6,16,41.8)→(123.6,5.7,16.7)·짧은 클릭 후 `flyTo` 호출 0(수정 전 1회 재비행)·비행 중 드래그 시 부감으로 튀지 않고 그 자리에서 회전. 씬은 WebGL 의존이라 순수 함수 테스트 대상 없음(브라우저 실측이 검증) |
| H20-15b | 주차 3D 좌드래그를 이동(팬)으로 | (사용자 재신고 2회) H20-15로 재비행·입력 씹힘은 풀렸는데도 축이 고정된다는 신고가 남았다 — 실체는 클로즈업 후 `OrbitControls.target`이 그 면에 묶여, 기본 좌드래그(=회전)가 항상 그 면을 축으로 도는 것. ①공용 씬 `packages/ui/src/components/parking-scene-3d/ParkingScene3D.ts`에 `mouseButtons={LEFT:PAN, MIDDLE:DOLLY, RIGHT:ROTATE}` · `touches={ONE:PAN, TWO:DOLLY_ROTATE}` — 좌드래그=이동, 우드래그·두 손가락=회전 ②`screenSpacePanning=false`로 팬이 화면이 아니라 바닥면을 따르게 ③소비 화면 2곳(`web-admin ParkingView3D.tsx`·`web-resident ParkingView3DResident.tsx`) 안내 문구 동기화 | 브라우저 실측(양 앱 — 좌드래그로 이동·우드래그로 회전) + 게이트 그린. 씬은 WebGL 의존이라 순수 함수 테스트 대상 없음 | ✅ 완료 — 공용 씬 1곳 수정으로 양 앱 동시 해결 |
| H20-16 | 관리자 AI비서 시설 질의 헛답 | (사용자 신고 2026-08-03 — 관리자 "404동 301호 콘센트는 어디에 있지?" → `get_facilities`로 라우팅 → 설비명 37개를 되풀이하는 루프 답변, 직전 "401동 201호…"는 no_evidence 폴백) 원인 둘: ⓐ근거(quote)에 설비명 20개를 나열해 8B에 반복 연료를 줬다 ⓑ세대 내부 설비 도구(`find_home_devices`·`trace_home_device_issue`)가 RESIDENT 전용이라 관리자에겐 대체 경로가 없다. ①`get_facilities` 근거를 **집계만**으로 교체 — 총수 + 상태별 + 종류별(계통 한글명은 공통 코드 `FACILITY_SYSTEM` LEFT JOIN, 약어만 주면 답변이 "EL: 12개"가 된다). 화면 카드(`data.items`)는 그대로 — 도구 name·description·인자 무변경(라우팅 파급 회피) ②관리자 채널 프롬프트 2종 + 도구 가시성 — 판정은 **라우터 `_admin_overrides`가 코드로** 한다(평면도 요소 어휘 `parse_query` ∧ 위치 어휘 ∧ 동·호수 유무, 사용자 지시 2026-08-03 "동/호수가 확인 안 되고 평면도의 시설일 경우만"): 동·호수 없으면 `ADMIN_AGENT_ASK_UNIT_PROMPT`로 **동/호수 되묻기**, 있으면 되묻지 않고 `get_facilities`를 **그 질의에서만 감춘다**(`answer_question(exclude_tools=…)` — 특정 세대 질문에 단지 37개 현황 카드가 뜨는 것이 신고 내용). 답변 프롬프트 규칙 7(`ADMIN_ANSWER_SYSTEM_PROMPT`)은 평면도 안내 문장. 그 외 관리자 질의는 공통 프롬프트 그대로. **되묻기 도구(`ask_clarification`) 설명은 손대지 않는다** — R31에서 막은 되묻기 도피가 다시 열린다. 세대 안 설비를 실제로 조회하는 관리자 도구는 **H20-17** | ai-core·api 단위 테스트(집계 근거·이름 미노출·분기 판정·프롬프트 배선) + 로컬 스택 실발화 눈검사(반복 루프·헛목록 0·동호수 명시 질의에 시설현황 카드 0) + 골든셋 시설 클래스 3회 중위 무회귀(되묻기 오남용 ≤6%·hard_fail 0) | ✅ 완료 — R40(로컬 8B, 시설 40건 전/후 ×3): 도구선택·인용 6회 전부 93.9% 동일, pass 중위 19=19, 오남용 0%, hard_fail 0. 눈검사에서 반복 루프·헛목록·시설현황 카드 소멸, 동·호수 없는 세대 설비 위치 질의는 되묻기 3/3 |
| H20-17 | 관리자 AI비서 "평면도 보기" | (사용자 확정 요구 2026-08-03 — "'402동 201호 두꺼비집 어디?' 물어보면 위치를 알려주고, 평면도 보기 버튼을 누르면 해당 평면도의 위치를 보여줘야 한다") H20-16의 스톱갭(트윈에서 확인하라는 안내)을 **실답변 + 화면 이동**으로 교체한다. ①관리자 도구 `find_household_devices`(ai-core `floor_plan.py` — 입주민 도구와 몸통 `_locate` 공유, 역할 `OFFICE_ROLES`): 조회 대상 세대는 **`ToolContext.target_unit`(라우터가 `parse_unit`으로 뽑은 값)만** 인정한다. 동·호수를 LLM 인자로 받지 않는 이유는 둘 — ⓐ마스킹 뒤라 모델 눈에는 `<PII:UNIT:1>`뿐이고(규칙 2) ⓑ세대 지정을 모델에 맡기면 착오가 곧 타 세대 조회다(규칙 4). 도구 **노출도 코드가** 정한다: `target_unit`이 없으면 스펙에서 감춘다(`_assistant_response`) — 다른 관리자 질의로 새는 경로를 없앤다 ②`_admin_overrides`가 4번째 값으로 대상 세대를 돌려주고, 동·호수 있는 갈래의 결정 프롬프트를 `ADMIN_AGENT_UNIT_PROMPT`(그 도구를 지목)로 교체. 답변 규칙 7은 "도구 결과가 있으면 그 방·방향을 옮기고, 없으면 트윈 안내"로 완화(빈손일 때도 지어내지 않는다) ③CTA "🏠 평면도 보기" → `/twin?dong=&ho=&device=` — 동·호수·강조 라벨은 **도구 카드 `data`(서버 확정값)에서만** 뽑고(`features/twin/assistant-links.ts`) URL 은 읽을 때 다시 검증한다. 트윈은 딥링크 세대를 1회 자동 선택(`matchHouseholdId`)하고 그 세대 평면도에만 강조를 건다 ④관리자 채널 골든셋은 러너(입주민 `/assistant/ask`) 밖이라 `admin-cases-draft.csv`로 분리 기록 — `gen`·채점 배선은 후속 | ai-core·api 단위(동·호수 필수·RESIDENT 차단·tenant 이중 방어·노출 조건) + web-admin 링크·매칭 단위 + 로컬 스택 실발화 4종×3회(세대 위치 답변·되묻기 유지·공용 설비 무회귀·입주민 무회귀) + 시설 클래스 40건 무회귀 | 🟡 구현 완료 · **측정 부분**(R41) — 실발화 6종×3회는 전부 통과(세대 위치 답변 3/3·미등록 세대는 폴백 3/3·되묻기 3/3·공용 설비 무회귀·입주민 무회귀), 시설 40건은 **1회 완주만**(도구 87.9%·hard_fail 0·신설 도구 호출 0, 중위 비교 미완 — 사용자 지시로 로컬 측정 중단, 잔여는 dev 실화면 확인). 이번 회차의 교훈은 규칙 7 문안이다: **탈출구 문장·예시 위치를 남기면 8B가 그리로 간다**(3안 기각 후 4안 채택, R41 ②) |
| H20-17b | 되묻기 후속 턴 라우팅 + 되묻기 도피 차단 | (dev 실화면 결함 2건) 원인 둘: ⓐ되묻기로 쪼개진 후속 턴("401동 201호")에는 설비 어휘가 없어 `_admin_overrides`가 풀리고 `get_facilities` 계통 집계 낭독이 부활 ⓑ도구가 미등록 세대라 note 를 반환하면 모델이 다음 스텝에서 `ask_clarification`으로 도피(동·호수가 이미 있는데 재차 되물음). ①직전 턴이 되묻기면 현재 질문을 원 질문 앞에 두고 합성해 판정(`_effective_question` — `parse_unit`이 첫 일치를 쓰므로 현재 턴이 우선), 합성은 되묻기 직후 **한 턴만** ②조회 대상 세대가 확정된 질의에서는 `ask_clarification`을 스펙에서 제외(`exclude_tools`) ③조회 질의(`ToolContext.target_query`)도 라우터 원문을 쓰고 LLM 보조 추출은 끈다 — 후속 턴 모델 인자엔 설비 어휘가 없다(규칙 2). 변경 지점: `apps/api/app/routers/assistant.py`·`packages/ai-core/.../tools/floor_plan.py`·`registry.py` + 테스트 | api·ai-core 단위 테스트 + dev vLLM 실측(성공 판정 3종 ×3회) + 시설 클래스 40건 무회귀 | ✅ 완료 — **R42**(dev vLLM qwen3-8b-awq): 등록 세대 정상 카드 3/3(1.3~1.4s) · 되묻기 후속 턴 2턴 라우팅 3/3(before 는 `get_facilities` 낭독, 5.0~5.4s) · `get_facilities` 무회귀 3/3 · 미등록 세대는 되묻기 오발동 대신 `fallback(no_evidence)` 3/3. 시설 40건 before/after 전 지표 동일(pass 32/40·도구선택 93.9%·인용 93.9%·되묻기 오남용 0%·hard_fail 0). 잔여: 다중 위치 답변에 `[1]` 인용 리터럴이 붙는 기존 잔여(R41과 같은 부류) |
| H20-18 | 관리비 **월 간 비교** | (사용자 신고 2026-08-03 — "관리비 7월 8월 비교해줘" → `no_evidence` 폴백) dev vLLM 재현 3/3으로 원인이 둘로 갈렸다: ⓐ모델이 달을 **`targets`에 넣는다**(`{"targets":"7월,8월","period":"7월,8월"}` — `fold_scope`가 "7월"을 동 이름으로 접어 데이터 없음) ⓑ여러 달을 나열할 때 **연도가 빠진다**("7월,8월" → `PERIOD_PATTERN` 검증 실패 = `invalid_args`, 오늘 날짜 주입(H19-11)으로도 안 막힘). 그리고 애초에 `requested_period()`가 여러 달을 max 한 달로 접어(H19-4 "비교 축은 대상") **월 간 비교 자체가 없었다**. ①`fees_common.fold_period_token` 신설 — "2026-07"·"2026-7"·"2026년 7월"·"7월"(·period 필드에서만 "7") → `YYYY-MM`, 연도 없으면 KST 올해. `GetFeesArgs.period`·`CompareFeesArgs.period` 양쪽에 걸어 **연도 없는 월을 공용 지점에서** 접는다(같은 결함을 부르는 형제 도구를 함께 고친다) ②`CompareFeesArgs`에 `model_validator` — `targets`의 월 토큰(**표식 필수**: "월" 또는 연도. 동 번호 "401"을 훔치지 않는다)을 `period`로 옮기고, `targets`는 optional로 완화(달끼리 비교면 대상이 안 남는다). 개수 경계(2~4)는 대상 축에서만 검사 ③서로 다른 달이 2개 이상이면 **비교 축을 기간으로 전환**: 대상 하나(지정 1개 · 없으면 본인 세대, 세대 미배정이면 단지 평균 — MANAGER 갈래)의 최근 두 달을 나란히. 셋 이상이면 최근 두 달만 쓰고 **버린 달을 quote에 적는다**(H19-4 교훈 — 조용히 빼지 않는다) ④카드: `title="2026-07 vs 2026-08 관리비 비교"`, quote는 두 달 확정값 + `증감 ±원(±%)`. **숫자는 전부 SQL 확정값**(`FEE_SQL`·`SCOPE_*_TOTAL_SQL` 재사용)이고 파이썬은 뺄셈 1회 + 퍼센트 1회만 한다(규칙 5 — AI는 계산 안 함). `data`는 `fee_compare` 그대로 두고(프론트 무변경 — 행 라벨이 월, 기준은 **나중 달**이라 UI 문장이 그대로 증감이 된다) `axis`·`subject`·`change_pct`만 additive ⑤FR-FEE-03(승인월 이전 차단)은 `_self_target`이 달마다 검사 — 한 달만 가능하면 그 달만 답하고 다른 달은 행 note. 도구 description은 `period`에 "여러 달을 주면 달끼리 비교" 한 구절만(라우팅 파급 회피 — 결정 프롬프트 무변경) | 단위(월 토큰 폴딩·targets→period 이동·두 달 비교·증감·퍼센트·승인월 경계·최근 두 달 절단·대상 축 무회귀) + dev vLLM 실발화 3종×3회(입주민 월 비교·"7월 관리비" 무회귀·관리자 월 비교=단지 평균) + 관리비·시설 골든셋 클래스 스모크 hard_fail 0 + 게이트 그린 | ✅ 완료 — R43(로컬 api + dev vLLM qwen3-8b-awq, before/after 교대): 실발화 3종×3회 전건 통과(월 비교 카드 `2026-07 vs 2026-08` + 증감 +7,077원(+3.9%) = DB 확정값 일치 · "7월 관리비" 무회귀 · 관리자는 단지 평균 축 3/3), before 3/3은 신고 증상 재현(`[compare_fees, get_fees]` → 1회 no_evidence·2회 "데이터가 제공되지 않아 비교할 수 없습니다"). 골든셋 관리비 44건 pass·인용·폴백·실패집합까지 before=after(43/44·97.7%·100%), 시설 40건은 1건 뒤집힘(0225 — 이번 변경이 손대지 않은 도구 경로, 8B 비결정성), hard_fail 0·되묻기 오남용 1.2%→0%. **원인 규명의 교훈**: 폴백만 보고는 못 고친다 — 결정 turn을 그대로 프로브해 인자를 뜬 게 원인 3건(달이 targets로·연도 누락·max 접기)을 갈랐다. 잔여: 월 비교 골든셋 케이스 없음(실발화·단위 테스트가 대신), `V2-QA-0337`은 `expected_tool=get_fees`인 낡은 라벨(before·after 모두 compare_fees로 라우팅 = H20-1 후속의 정답 경로), 연도 없는 월은 KST 올해로 읽어 연말연시 발화는 후속 후보 |
