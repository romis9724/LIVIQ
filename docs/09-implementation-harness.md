...
| H15. LLM 백엔드 런타임 전환 | SYS_ADMIN AI 설정 UI(base URL·모델·API 키·연결 테스트)·`ai_backend_config` 전역 테이블·요청 단위 즉시 반영(env 폴백)·백엔드 비교 측정(ollama vs vLLM vs OpenAI) | SYS_ADMIN 인가(CRITICAL)·API 키 마스킹 응답 + 캐시 키 백엔드 분리 + 게이트 그린 + 시각 실측 | 🚧 진행 (§8.18 — H15-1 ✅·H15-3 ✅, H15-2 로컬 축 완료·OpenAI 축 대기) |
