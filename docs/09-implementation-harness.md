...
| 순서 | 작업 | 산출물 | 완료 기준 | 상태 |
|------|------|--------|-----------|------|
| H2-0 | 설계 갱신 | REST API 표면([01 §13](01-architecture.md)) 신설 · `inquiry_events`·검수 필드([03 §4.3·4.4](03-database-design.md)) · 신규 ADR 불필요(기존 0006·0011·0012 커버) | 설계 문서 PR 머지(구현 착수 전) | PR 검토 중 |
| H2-1 | 인증·세션·역할 | Redis 서버 세션([ADR-0011](adr/0011-redis-server-session.md))·로그인·역할(resident/admin) 인가 미들웨어, dev 헤더는 local 전용으로 격리 | 로그인→세션→역할별 엔드포인트 인가 테스트(CRITICAL), 교차 역할 접근 거부 | 대기 |
| H2-2 | 문서 관리 화면 실연동 | web-admin documents 화면 → 업로드·인제스트 상태 api 연동(백엔드는 H1-4 완성) | 업로드→indexing→indexed 상태가 화면에 반영 | 대기 |
| H2-3 | 민원(inquiries) | 접수·목록·상태 타임라인 api + web-resident 실연동. AI 분류·요약은 **초안**(검수 대상, 규칙 6) | 접수→조회→상태 변경 E2E 그린 | 대기 |
| H2-4 | 공지 초안 | 키워드→AI 초안 생성 api(출처 인용 강제) + web-admin 스테퍼 실연동. **발송은 사람 확정**(자동발송 금지) | 초안에 출처 동반, 발송 버튼은 사람 확인 후에만 활성 | 대기 |
| H2-5 | 관리비 | 엑셀 업로드→검증→확정 데이터 적재([ADR-0006](adr/0006-fees-excel-upload-source.md)) + 조회 api + AI 설명(**설명만, 계산 금지 — 규칙 5**) + web 실연동 | 업로드→조회 정합 테스트, AI 응답에 확정 데이터 출처 | 대기 |
| H2-6 | 검수 큐 | `needs_review` 메시지 큐 api + web-admin review-queue 실연동(승인/반려) | 저신뢰 답변이 큐에 적재→처리되는 흐름 테스트 | 대기 |
