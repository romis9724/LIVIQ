...
```bash
uv sync                           # 루트 uv workspace — 전 Python 패키지 의존성 잠금 설치
```
...
| 2 | uv workspace 초기화 | 루트 uv workspace · Python 패키지 골격(api·ai-worker·ai-core·db) · 얇은 package.json(turbo 연결) | `uv sync` 성공, turbo가 Python 태스크 인식 |
