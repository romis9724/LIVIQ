...
| H19-7 | 민원 접수 CTA·복귀 동선 | (사용자 지적 — 접수 CTA가 다른 CTA와 이질적이고, 넘어간 화면에서 AI 비서로 돌아올 길이 없다) ①"민원 접수하기" CTA에 아이콘 추가(🅿️ 주차·🏠 평면도 관례와 통일) ②`/inquiries`를 어시스턴트 딥링크(`?compose=1`)로 열면 **상단 뒤로가기** 표시(`router.back()` — [FloorPlanView](../apps/web-resident/src/features/floor-plan/FloorPlanView.tsx)·ParkingMapView 관례) → AI 비서 대화로 복귀(대화는 sessionStorage 복원 — H17 UI 정리) ③하단 "민원" 탭으로 직접 진입하면 뒤로가기 **비노출**(딥링크 param 유무로 판정, 마운트 시 1회 — 프리필과 동일 규칙) | compose 딥링크에서만 뒤로가기 렌더(탭 진입 미렌더 테스트) + 프리필·IME 회귀 없음 + 게이트 그린 + 시각 검증(스크린샷) | ⬜ 대기 |
