...
- **DTO-first**: 모든 경계 계약은 Zod 스키마로 `packages/shared`에 먼저 정의하고 web·api가 import(단일 출처, [02 §7](02-directory-structure.md)).
- assistant 응답 **스트리밍은 SSE** — 이벤트 4종: `token`(증분 텍스트) · `citation`(근거 카드) · `status`(단계·도구 진행) · `done`(종료·최종 신뢰도).
- 화면별 핵심 엔드포인트는 **H1에서 `packages/shared`에 확정**한다(그 전엔 계약만 두고, 구현은 빌드 순서대로 채운다).
...
- env는 `.env`(로컬), [`.env.example`](../.env.example)(레포 루트) 제공. 부팅 시 Zod 검증(누락=즉시 실패). **검증 소유는 패키지별**(.env.example 주석) — `packages/db`가 `DATABASE_URL`, `apps/api`가 세션·S3·인증, `packages/ai-core`가 LLM·임베딩.
...
| drizzle-orm / drizzle-kit | 최신 stable | 스키마·마이그레이션 |
| PostgreSQL | 16 (pgvector) | compose 이미지와 일치 |

- **RLS 정책 SQL은 drizzle custom migration으로 버전 관리**한다 — 스키마 push가 생성하지 못하는 정책·role을 마이그레이션 파일로 고정(코드 리뷰가 아니라 마이그레이션 이력으로 추적).
...
| 커버리지 | 패키지별 threshold **80%**(라인/브랜치), 미달 = 실패 |
| 범위 | turbo `--filter=...[origin/main]` — 변경 영향 패키지만 |
| 시크릿 스캔 | **gitleaks** (PR diff + 전체 히스토리) |
| 경로 검증 | `node scripts/check-context-paths.mjs` (stale 링크 차단) |

### 4.2 테스트 하네스

- 통합 테스트는 **Testcontainers로 PostgreSQL 기동** — 실제 마이그레이션·RLS를 적용해 검증(모킹 아님).
- **역할 2개**: 마이그레이션 owner role(DDL·정책 생성) + 런타임 role(**BYPASSRLS 없음** — RLS를 실제로 받는다). 워커도 런타임 role.
- 격리는 **트랜잭션 롤백**(각 테스트를 트랜잭션으로 감싸 종료 시 롤백 — 컨테이너 재기동 없이 빠르게).
...
| 2 | packages/db 골격 + `CREATE EXTENSION vector` | 스키마·첫 마이그레이션 | 마이그레이션 적용, vector 확장 활성 |
| 3 | env Zod 검증 (packages/db·apps/api 각자 소유) | config 모듈 | 누락 env = 부팅 실패 |
| 4 | RLS 정책 + 워커 role | `rls/` SQL(custom migration), 런타임 role | 교차 tenant 접근 거부 테스트 통과(CRITICAL) |
| 5 | Testcontainers 픽스처 | PG 기동·마이그레이션·트랜잭션 롤백(§4.2) | 통합 테스트 그린 |
| 6 | `ci.yml` | 게이트 워크플로(§4.1) | PR에서 전 단계 실행 |
| 7 | 빈 apps/api 그린 빌드 | 부팅되는 NestJS | 헬스체크 200, 그린 빌드 |
