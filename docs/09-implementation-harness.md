...
| get_dek 최초 생성 경합(uq 위반) | [ADR-0010](adr/0010-envelope-encryption-env-master-key.md) | H6-4 E2E가 최초 트리거 — 신규 tenant 첫 동시 요청 2건이 tenant_keys(v1)를 read-then-insert 경합. upsert(ON CONFLICT) 원자화 + 동시성 pytest 필요(crypto 경로 — 신중히) |
| 동의 변경·설정 토글 서버 연동 | [04 §2](04-menu-structure.md) 나 화면 | H6-3은 표시 전용 — 동의 변경 API 부재. 수요 확인 후 |
| OAuth 콜백 앱별 복귀 | H6-1 | 콜백이 WEB_BASE_URL(입주민) 단일 복귀 — 쿠키 공유라 동작 무지장, 관리자 UX 개선은 후속 |
