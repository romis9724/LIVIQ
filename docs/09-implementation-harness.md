...
| H7-9 | 주민 관리 개편 | 운영자 인터뷰(2026-07-22): 명부가 쓰기 전용(업로드 결과만 반짝)이라 상태 확인 불가 → **가입 승인 화면을 '주민 관리'로 개편(목록 위주 UX)**. ①`GET /admin/roster` — 명부 목록(성함 마스킹·생년 비표시, 상태=미가입/가입완료(소진)/전출후보, 검색·페이지네이션)+총계+마지막 업로드 요약 ②승인 카드 **불일치 사유** 구체화(세대 없음/인적 불일치/이미 소진) ③메뉴·라우트 `/approvals`→`/residents` | 명부 상태 분류·검색 테스트 + 시각 실측 + E2E 갱신 그린 | 진행 중 |
