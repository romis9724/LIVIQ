...
| H20-16 | 관리자 AI비서 시설 질의 헛답 | (사용자 신고 2026-08-03 — 관리자 "404동 301호 콘센트는 어디에 있지?" → `get_facilities`로 라우팅 → 설비명 37개를 되풀이하는 루프 답변, 직전 "401동 201호…"는 no_evidence 폴백) 원인 둘: ⓐ근거(quote)에 설비명 20개를 나열해 8B에 반복 연료를 줬다 ⓑ세대 내부 설비 도구(`find_home_devices`·`trace_home_device_issue`)가 RESIDENT 전용이라 관리자에겐 대체 경로가 없다. ①`get_facilities` 근거를 **집계만**으로 교체 — 총수 + 상태별 + 종류별(계통 한글명은 공통 코드 `FACILITY_SYSTEM` LEFT JOIN, 약어만 주면 답변이 "EL: 12개"가 된다). 화면 카드(`data.items`)는 그대로 — 도구 name·description·인자 무변경(라우팅 파급 회피) ②`ADMIN_ANSWER_SYSTEM_PROMPT` 규칙 7 — 세대 안 설비 위치 질문은 NO_EVIDENCE도 공용 설비 목록도 아닌 **트윈 대시보드 세대 평면도 안내**(어느 세대인지 없으면 되묻기). `channel="admin"`에만 주입 — 입주민은 본인 세대 도구가 있어 이 안내가 틀린 답이다. **되묻기 도구(`ask_clarification`)는 손대지 않는다** — R31에서 막은 되묻기 도피가 다시 열린다 | ai-core·api 단위 테스트(집계 근거·이름 미노출·프롬프트 배선) + 로컬 스택 실발화 눈검사(반복 루프·헛목록 0) + 골든셋 시설 클래스 3회 중위 무회귀(되묻기 오남용 ≤6%·hard_fail 0) | ✅ 구현 완료 — 측정 R40(로컬 8B, 시설 40건 전/후 ×3) |
