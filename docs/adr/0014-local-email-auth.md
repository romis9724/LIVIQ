...
- **메일 발송**: 어댑터 인터페이스(Protocol) 뒤 — `MAIL_BACKEND=console|smtp`(local 기본 console — 링크 로그 출력), SMTP는 env(`SMTP_HOST/PORT/USER/PASSWORD/FROM`). 프로바이더 선정은 파일럿 배포 시.
...
- **메일 인프라 필요**: 검증·초대·재설정이 메일 발송에 의존 — local은 console 어댑터로 대체(링크 로그 출력), 운영은 SMTP. 프로바이더 미선정은 어댑터 뒤로 미룸.
