...
## 개정 노트 2 (2026-07-27, H12-2·H12-3 실배포)

**결정 4의 레지스트리를 GitLab → Nexus 로 바꾼다.** 그리고 외부 접속 방식이 추가됐다.

- **레지스트리 = 사내 Nexus 의 docker hosted 저장소**(`192.168.10.153:8082/liviq/<앱>:<sha>`).
  GitLab 컨테이너 레지스트리를 **쓸 수 없다는 것이 실측으로 드러났다** — 그 GitLab 은 docker
  컨테이너이고 공개 포트가 22·80·443 뿐이라 **레지스트리 포트(5000)가 노출돼 있지 않다**.
  `:5050`(웹 포트)으로 push 하면 `/v2/` 가 dependency proxy 로 응답한다. 컨테이너 안에서 registry
  프로세스는 돌고 있어 포트만 공개하면 되지만, 컨테이너 재생성이 필요하고 이미지가 `/` 파티션에
  쌓인다(그 호스트의 `/` 는 당시 100%였다). 사내에 Nexus 가 이미 있어 그쪽을 쓴다(사용자 결정).
  - 태그 핀(`$CI_COMMIT_SHORT_SHA`)·`latest` 비배포 원칙은 **불변**. 좌표만 바뀐다.
  - `publish` 의 `allow_failure` 를 제거했다 — 게시 실패는 "롤백 백업 없음"이므로 빨개져야 한다.
  - 보존 정책 30일([13 §8.3](../13-gitlab-wsl-deploy.md)). 배포가 로컬 이미지로 이뤄져 게시본은
    pull 되지 않으므로 "마지막 다운로드 이후 30일"이 곧 백업 보존창이 된다.
- **외부 접속은 포트 기반 사이트 주소**로 받는다(도메인 없음). Caddy 에 `:3000`·`:3001` 주소를
  하나 더 붙이고 3단 포워딩(bastion 17000/17001 → Windows 3000/3001 → WSL)을 탄다. 평문 HTTP 라
  **가상 데이터 파일럿 전용**이다([13 §9](../13-gitlab-wsl-deploy.md)).
- **롤백 실연 완료**: `083c18d7-vworld` 로 되돌린 뒤 현재로 복귀. 코드는 되돌아가고(신규 UI 문자열
  0건) 데이터·스키마는 유지됐다(households 322 · alembic head 불변) — ADR-0020 이 전제한 성질이
  이 형상에서도 성립함을 확인했다.
- **운영 부담 하나 확정**: docker `log-opts` 는 컨테이너 **생성 시점에 고정**되어 기존 컨테이너에
  적용되지 않는다. 두 호스트에 `logrotate`(copytruncate) 규칙을 넣어 막았다([13 §8.4](../13-gitlab-wsl-deploy.md)).
