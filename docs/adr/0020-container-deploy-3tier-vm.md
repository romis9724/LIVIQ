...
> **개정 노트(2026-07-26, ADR-0021)**: 본 ADR은 배포 대상을 3-tier VM + GHCR **하나**로 서술하지만,
> 사내 단일 호스트(Windows Server + WSL2 Docker) + GitLab CI·레지스트리 형상이 추가됐다 —
> [ADR-0021](0021-gitlab-ci-single-host-wsl.md). 본 ADR의 결정 1·4·5·6(이미지·프록시·마이그레이션·LLM)은
> 두 형상 공통이고, **결정 2(3-tier 네트워크 방어선)는 단일 호스트 형상에서 성립하지 않는다**.
> 결정 3(compose 1개 + profiles)이 두 형상을 한 파일로 지원하는 근거가 됐다. 이미지 좌표 규약은
> ADR-0021 결정 5에서 변경됐다 — `IMAGE_PREFIX`가 **구분자까지 포함**한다(GitLab 레지스트리는
> 하위 저장소를 슬래시로만 표현하므로 대시 하드코딩이 불가).
