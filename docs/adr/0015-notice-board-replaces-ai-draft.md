...
> **개정 노트(2026-07-25)**: 위 "마스킹 미적용"을 **철회**한다 — 문서·공지 **양쪽 모두 임베딩 직전 마스킹**(fail-closed). 근거 두 가지. ① 인용한 "documents 인제스트 선례"가 애초에 결함이었다: `embed_chunks`가 청크 **원문**을 임베딩 엔드포인트로 보내고 있었고, 이는 [docs/06 §4.2](../06-security-privacy.md)가 이미 요구한 "LLM/**임베딩** 호출 직전 마스킹"을 코드가 위반한 상태였다(설계 변경이 아니라 설계 위반 수정). ② "공개 문서라 추가 노출 없음"은 규칙 2의 판단 기준이 아니다 — 규칙 2가 금지하는 것은 입주민 노출이 아니라 **LLM 프로바이더로의 전송**이고, 공지 본문에도 개인정보("101동 홍길동님 택배")가 들어온다. 구현: 마스킹을 공통 초크포인트 `embed_chunks` 한 곳에 두고(호출자별 가드는 새 호출자에서 누락된다) 실패 시 문서=`index_status=failed`·공지=`status=failed`로 색인 중단, DB 청크 본문은 원문 유지(RLS 보호·인용 표시에 필요). **대가**: PII를 포함한 텍스트는 플레이스홀더로 임베딩되어 해당 부분의 검색 재현율이 떨어진다 — 규칙 2가 우선한다.
